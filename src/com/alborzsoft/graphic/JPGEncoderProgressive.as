...
{
	/**<p>Progressive JPGEncoder to prevnt the frizing the app since encoding</p>
	...
	[Event(name="complete", type="com.alborzsoft.graphic.events.ImageResultEvent")]
	 ...
	 */		
	public class JPGEncoderProgressive extends EventDispatcher
	{
		 ...
		 * @tiptext
		 */		
		public function JPGEncoderProgressive(quality:Number = 50)
		{
			if (quality <= 0) {
				quality = 1;
			}
		 ...
		 * @param delay - uint of Delay between processing each  that indicates the Wuality of JPG image
		 * @return a ByteArray representing the JPEG encoded image data.
		 * @langversion ActionScript 3.0
		 * @playerversion Flash 9.0
		 * @tiptext
		 */	
		public function encode(image:BitmapData, delay:uint=10):void
		{
			...
			var ypos:int = 0;
			var mul:int = 8;
			var xtimer:Timer = new Timer(delay, (image.height/mul)+1);
			xtimer.start();
			xtimer.addEventListener(TimerEvent.TIMER, function():void
			{
				xtimer.stop();
				
				for (var xpos:int=0; xpos<image.width; xpos+=mul) {
				...
				dispatchEvent(new ProgressEvent('progress', false, false, (ypos+mul), image.height));
				xtimer.start();
			});
			
			
			xtimer.addEventListener(TimerEvent.TIMER_COMPLETE, function():void{
				...
				dispatchEvent(new ImageResultEvent(ImageResultEvent.COMPLETE, false, false, byteout));
			});
		}
